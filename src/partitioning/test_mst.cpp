...
int main(int argc, char** argv) {
    ...
    std::cout << "Running Stage 2 Strategy #2 (MST Clustering)..." << std::endl;
    std::ofstream out("mst_clusters_output.csv");
    out << "original_id,x,y,macro_id,micro_id\n";
    out << depot.original_id << "," << depot.x << "," << depot.y << ",-1,-1\n";
    
    int global_micro_id = 0;
    MSTClusterer micro_clusterer;
    
    for (size_t i = 0; i < wedges.size(); ++i) {
        std::cout << "  -> Computing MST for Wedge " << i + 1 << " (" << wedges[i].size() << " nodes)..." << std::endl;
    ...
    std::cout << "Generated total " << global_micro_id << " MST micro-clusters." << std::endl;
    std::cout << "Output saved to mst_clusters_output.csv!" << std::endl;
