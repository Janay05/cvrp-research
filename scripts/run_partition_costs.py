...
def main():
    ...
    for folder in partition_folders:
        ...
        for c in all_chunk_files:
            ...
            # OUTER IF: Check if it exists in the CSV at all
            if row and len(row) >= 3:
                # INNER IF: Check if it was a success so we can add to our totals
                if str(row[2]) not in ("ERROR", "INCOMPLETE"):
                    try:
            ...
            # OUTER ELSE: Only run it if it was NEVER recorded in the CSV
            else:
