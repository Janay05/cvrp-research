#!/bin/bash
# 5-seed VDA check of the depth-2 ejection chain operator, same config as every other VDA
# 5-seed benchmark this session, isolated against the "no eject2" baseline
# (rev_variants_vda_5seed, mean 21,773,253).
set -e
cd /mnt/c/internship/iitm/cvrp
OUT=results/bench/eject2_vda_5seed
mkdir -p "$OUT"
