...
**Scale testing ([report 003](docs/reports/003_scale_testing.md)):** verified correct and feasible from 50 to ~1,000,000 customers (CMT1 → Valle-D-Aosta → Lazio, the same instance family FILO2 ships). At 50 nodes the solver is essentially optimal against the published best-known solution (525 vs. 524.611). **At 1,000,000 nodes, the solver beats FILO2 by 2.4x on wall-clock (264s vs. 641s) while staying within 1.1% on cost, using 10x fewer iterations** — the strongest evidence so far that the chunking architecture's advantage compounds at the scale it was actually designed for. The one open item: the per-thread iteration-budget formula from report 002 was tuned at N=2000 and doesn't generalize as-is to larger N (confirmed inefficient at 20k, unusable unmodified at 1M) — it needs to become scale-aware next.
